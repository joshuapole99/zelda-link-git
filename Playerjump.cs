...
public class Playerjump : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            Debug.Log("werkt het");
            if (Input.GetKeyDown(KeyCode.Q) && Physics.Raycast(transform.position, Vector3.down, 1))
            {
                // _rigidbody.AddForce(new Vector3(-60, 35, 0) * JumpLeft);
              ...
              //  transform.Translate(Vector3.left * Speed * Time.deltaTime);
                _rigidbody.AddForce(up * 5, ForceMode.Impulse);
                _rigidbody.AddForce(left * 5, ForceMode.Impulse);
            }

            if (Input.GetKeyUp(KeyCode.Q) && Physics.Raycast(transform.position, Vector3.down, 1))
            {
                _rigidbody.AddForce(Vector3.down * JumpStop);
            }


            if (Input.GetKeyDown(KeyCode.E) && Physics.Raycast(transform.position, Vector3.down, 1))
            {
                // _rigidbody.AddForce(new Vector3(60, 35, 0) * JumpRight);
                ...
                //  transform.Translate(Vector3.left * Speed * Time.deltaTime);
                _rigidbody.AddForce(up * 5, ForceMode.Impulse);
                _rigidbody.AddForce(right * 5, ForceMode.Impulse);
            }


            if (Input.GetKeyUp(KeyCode.E) && Physics.Raycast(transform.position, Vector3.down, 1))
            {
                _rigidbody.AddForce(Vector3.down * JumpStop);
            }
            jumping = false;
